...
const FrontPage = withTheme(({ theme }: ThemedProps) => (
  <Flex as="article" direction="column">
    ...
    <H2>To Do</H2>
    <Paragraph>
      The thing that is in particular lacking are correct/good ways to do layouts. Hence, work on container & layout
      components.
    </Paragraph>
    <Paragraph>
      Theming in particular feels a bit clunky. Can I simplify / improve this? In particular, can I remove some of the
      behaviours and separate them out into different files?
    </Paragraph>
    <Paragraph>I'm lacking UI interface colors, e.g. lots of greys for the chrome.</Paragraph>
    <Paragraph>
      Transitions and animations need to be done ad-hoc at the moment. How do I inject these styles for systematically?
    </Paragraph>
    <Paragraph>Can I find a better / new way to organise/name elements and values, so it follows a system?</Paragraph>
    ...
    <Paragraph>Responsiveness is clunky now. How do you realise this in a good way?</Paragraph>
  </Flex>
));
...
const HeadingsPage = withTheme(({ theme }: ThemedProps) => (
  <Flex as="article" direction="column">
    ...
    <Paragraph>
      You can also use the shorthands <Code>H1</Code> to <Code>H6</Code> to render the corresponding tags.
    </Paragraph>
